...
test('Parent Contacts FO', async ({ page }, testInfo) => {
  ...
  const myIdEmail = getMyIdEmail('ParentContactsFO.spec.ts');
  ...
  if (!bysVisible) {
    throw new Error(
      'Auth session is not valid for ParentContactsFO. Run auth setup first: npx playwright test tests/auth.setup.ts --project=setup --headed'
    );
  }
