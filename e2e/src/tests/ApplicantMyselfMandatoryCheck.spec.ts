...
test('Applicant Myself Mandatory Check', async ({ page }, testInfo) => {
  ...
  const myIdEmail = getMyIdEmail('ApplicantMyselfMandatoryCheck.spec.ts');
  ...
  if (!bysVisible) {
    throw new Error(
      'Auth session is not valid for ApplicantMyselfMandatoryCheck. Run auth setup first: npx playwright test tests/auth.setup.ts --project=setup --headed'
    );
  }
