...
test('Parent Contacts SFM', async ({ page }, testInfo) => {
  ...
  const loginIdentity = getLoginIdentityForSpec('ParentContactsSFM.spec.ts');
  ...
  const ensureBysWithFreshMyIdLogin = async () => {
    ...
    if (!bysVisible) {
      throw new Error(
        `Auth session is not valid for Parent Contacts SFM after full ${loginIdentity.provider} login flow. ` +
