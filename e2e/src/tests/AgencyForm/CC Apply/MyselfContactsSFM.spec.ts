...
test('Myself Contacts SFM', async ({ page }, testInfo) => {
  ...
  const loginIdentity = getLoginIdentityForSpec('MyselfContactsSFM.spec.ts');
  ...
  const ensureBysWithFreshMyIdLogin = async () => {
    ...
    if (!bysVisible) {
      throw new Error(
        `Auth session is not valid for Myself Contacts SFM after full ${loginIdentity.provider} login flow. ` +
  ...
  const screenshotPath = path.join(screenshotDir, `MyselfContactsSFM_ContactDetails_${timestamp}.png`);
  await page.screenshot({ path: screenshotPath, fullPage: true });
  console.log('Myself Contacts SFM Contact Details screenshot saved at: ' + screenshotPath);
