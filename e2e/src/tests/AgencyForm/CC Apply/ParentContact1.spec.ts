...
test('Parent Contact1', async ({ page }, testInfo) => {
  ...
  const loginIdentity = getLoginIdentityForSpec('ParentContact1.spec.ts');
  ...
  const ensureBysWithFreshMyIdLogin = async () => {
    ...
    if (!bysVisible) {
      throw new Error(
        `Auth session is not valid for Parent Contact1 after full ${loginIdentity.provider} login flow. ` +
