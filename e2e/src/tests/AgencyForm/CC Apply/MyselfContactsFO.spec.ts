...
test('Myself Contacts FO', async ({ page }, testInfo) => {
  ...
  const screenshotPath = path.join(screenshotDir, `MyselfContactsFO_ContactDetails_${timestamp}.png`);
  await page.screenshot({ path: screenshotPath, fullPage: true });
  console.log('Myself Contacts FO Contact Details screenshot saved at: ' + screenshotPath);
