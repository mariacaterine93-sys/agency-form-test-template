...
test('Myself Contacts PLG', async ({ page }, testInfo) => {
  ...
  const myIdEmail = getMyIdEmail('MyselfContactsPLG.spec.ts');
  ...
  if (!bysVisible) {
    throw new Error(
      'Auth session is not valid for MyselfContactsPLG. Run auth setup first: npx playwright test tests/auth.setup.ts --project=setup --headed'
    );
  }
  ...
  await page.screenshot({
    path: testInfo.outputPath('myself-contacts-plg-contact-details.png'),
    fullPage: true,
  });
