...
test('Review Myself 2 Contacts', async ({ page }, testInfo) => {
  ...
  const myIdEmail = getMyIdEmail('ReviewMyself2Contacts.spec.ts');
  ...
  if (!bysVisible) {
    throw new Error(
      'Auth session is not valid for ReviewMyself2Contacts. Run auth setup first: npx playwright test tests/auth.setup.ts --project=setup --headed'
    );
  }
