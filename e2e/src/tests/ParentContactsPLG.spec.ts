...
test('Parent Contacts PLG', async ({ page }, testInfo) => {
  ...
  const myIdEmail = getMyIdEmail('ParentContactsPLG.spec.ts');
  ...
  if (!bysVisible) {
    throw new Error(
      'Auth session is not valid for ParentContactsPLG. Run auth setup first: npx playwright test tests/auth.setup.ts --project=setup --headed'
    );
  }
  ...
  await page.screenshot({
    path: testInfo.outputPath('parent-contacts-plg-contact-details.png'),
    fullPage: true,
  });
